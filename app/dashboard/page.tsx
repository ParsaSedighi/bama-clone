...
import { getMyAdvertisements, deleteAdvertisement } from "@/app/actions/advertisementActions";
    ...
    return (
        <main className="container mx-auto px-4 py-8">
            ...
            <div className="flex justify-between items-center mb-6">
                ...
                <Button asChild><Link href="/ads/create">Post New Ad</Link></Button>
            </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        {myAds.map((ad) => (
                            <Card key={ad.id} className="flex flex-col">
                                <Link href={`/ad/${ad.id}`} className="flex-grow">
                    ...
                    <div className="space-y-4">
                        {mySales.map((sale) => (
                            <Card key={sale.id}>
                                ...
                                <CardContent>
                                    <p><strong>Buyer:</strong> {sale.buyer.name}</p><p><strong>Status:</strong> {sale.status}</p>
                                    <TransactionSellerActions transactionId={sale.id} transactionStatus={sale.status} acceptAction={acceptTransaction.bind(null, sale.id)} rejectAction={rejectTransaction.bind(null, sale.id)} />
                                </CardContent>
                            </Card>
                        ))}
                    </div>
                    ...
                    <div className="space-y-4">
                        {myPurchases.map((purchase) => (
                            <Card key={purchase.id}>
                                ...
                                <CardContent>
                                    <p><strong>Seller:</strong> {purchase.advertisement.user.name}</p><p><strong>Status:</strong> {purchase.status}</p>
                                    <TransactionBuyerActions transactionStatus={purchase.status} completeAction={completeTransaction.bind(null, purchase.id)} />
                                </CardContent>
                            </Card>
                        ))}
                    </div>
